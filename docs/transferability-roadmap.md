...
This roadmap tracks the public, beginner-friendly pieces of a safer AI coding workflow.

The goal is to make useful agent habits portable without copying private machines, secrets, auth state, logs, local memory, hooks, MCP configs, or global trust settings.

## Core Thesis

Vibe coding becomes more reliable when the agent has a small operating layer:
...
This repo is the first public slice of that operating layer.
...
1. Add a dedicated setup-auditor prompt that produces the scanner report format.
2. Add examples for Cursor, Claude Code, Codex CLI, and generic chat-based coding agents.
3. Add an interview/demo walkthrough with before/after transcripts.
4. Add a friend-audit checklist for reviewers trying this repo cold.
5. Decide whether the scanner remains prompt-only or becomes an optional read-only script.
6. If a script is added later, keep it read-only by default and require explicit file allowlists.
